...
| 2026-09-08 | `AdesaoService.steps`/`wizardSteps` viraram `computed()` (antes eram array estático `readonly steps: AdesaoStep[]`); nova etapa "Seleção de plano" como etapa 1 do fluxo; etapa "Vínculo" suprimida para plano Instituído; nova etapa "Representantes" inserida após "Dados pessoais" quando titular é menor de idade; `verificacao-cpf` ganhou checagem de elegibilidade de patrocinadora para plano Patrocinado | Requisito de negócio: fluxo condicional por tipo de plano e por menoridade do titular, com numeração do stepper recalculada dinamicamente sem lacunas (RN09/RN13). O offset fixo `index + 3` em `AdesaoLayout.goToPanelStep()` (introduzido em 2026-07-23) foi substituído por busca via `panel.activeSubStep`, já que a posição das etapas deixou de ser fixa — `sobre-voce` e `verificacao-email` continuam nas mesmas posições relativas de antes (fora do stepper numerado), só que agora sem depender de aritmética de índice. Novos mocks: `data/planos-mock.data.ts` (Plano, elegibilidade de Patrocinado) e `data/relacoes-menor-mock.data.ts` (relação→documento aceito) |
| 2026-09-10 | Etapa `sobre-voce` removida do wizard (rota, `SOBRE_VOCE_STEP` em `adesao.service.ts` e a página); `senha-acesso` agora segue direto para a próxima etapa de dados (`vinculo`/`dados-pessoais`) | Pedido do usuário: a etapa era percebida como redundante, já que a etapa seguinte de dados já pede nome/telefone novamente. Reverte a decisão de 2026-07-23 que a introduziu para captura antecipada de lead — trade-off aceito explicitamente pelo usuário ao ser alertado sobre a perda dessa captura |
| 2026-09-10 | `Representantes`: layout de card (`&__bloco`) removido dos blocos "Representante legal" e "Representante financeiro", padronizado sem card como a maioria das telas do wizard (regra geral de §7, não uma exceção de §7.5); os dois blocos agora são separados por um `<hr>` (`&__divider`) em vez de bordas individuais. `validarRepresentante()` em `representantes.ts` passou a exigir nome, data de nascimento, sexo, estado civil, escolaridade, telefone e e-mail preenchidos (antes só validava CPF, relação, idade e documento) | Auditoria a pedido do usuário: os campos eram marcados `[required]` visualmente mas não bloqueavam o avanço do wizard se deixados em branco — só relação/CPF/idade/documento eram de fato validados em `avancar()` |
| 2026-09-10 | Novas máscaras `telefone` e `data` em `LemeTextFieldComponent` (`mask="telefone"`/`mask="data"`, config em `leme-text-field-masks.ts`), aplicadas aos campos "Data de nascimento" e "Telefone celular" de `representantes.html`. `formatPhoneInput` (duplicado em `shared/utils/phone-format.util.ts`, usado só pela extinta `sobre-voce`) foi removido | Mesmo padrão já usado para a máscara de CPF (decisão de 2026-07-23): formatação de campo é responsabilidade do Leme, não de lógica ad-hoc por página. `representantes.html` tinha os dois campos sem nenhuma máscara |
...
| 2026-09-14 | Categoria "cartão único centralizado" (§7.5) passa a ter card visível (`&__card`: borda + fundo branco + padding) em **todas** as telas, incluindo `verificacao-cpf` e `senha-acesso`, que antes não tinham. Conteúdo interno dessas duas continua alinhado à esquerda, sem ícone circular — não foi replicado o tratamento centralizado de `retomar-adesao`/`recuperar-senha`/`conclusao`/`acompanhamento` | Auditoria de conformidade a pedido do usuário (Alberto, via print de revisão): das 7 telas da categoria, 4 já tinham card e 3 não — levado a votação, o padrão majoritário (com card) venceu. Escopo relatado como "card mínimo": só a caixa, sem replicar centralização/ícone, para não ampliar a mudança além do que foi pedido |
